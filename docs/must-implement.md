...
**Admission (binary):** published Marcaj + all jobs submitted + `route.geojson` / `measurements.csv` in the required formats + working web UI.
...
| Walking route | 25% | `siret3 route` + sample 4326 geojson | Legal graph: inter-rows ∪ `passages.geojson` − `forbidden.geojson`; visit waste + inspections within 2 m; closed at official start ± 5 m; `length_m`; **coordinates EPSG:32635** |
| Web interface | admission + 15% engineering | Next.js MapLibre on :43173, synthetic layers | Real layers: blocks, rows, inter-rows, waste, inspections, route, lengths, counts |
...
Install → tiles → model → CVAT ZIPs → Marcaj note → export → `measurements.csv` + `route.geojson`. Pinned deps. Dockerfile plus. Weight URL. Full-set time + hardware. Paid APIs/LLMs. Web URL.
...
siret3 inspect       →  inspections.geojson   (gaps + waste targets)
siret3 route         →  route.geojson         (32635, closed, legal)
        │
        ▼
web/  layers + counts + lengths
```

Inspection command does not exist yet. Route must consume official start, passages, forbidden.
...
- [ ] Targets = waste centroids + gap/missing-planting points (`id`, coords, `vineyard_id`, `row_id`)
- [ ] Closed walk, start snap ≤ 5 m, visit within 2 m, minimise length after coverage
- [ ] Reject (fail loud) if &gt; 2% length is illegal
- [ ] Write `route.geojson` LineString in **EPSG:32635** with `length_m`
...
- [ ] Web: official start, real layers, IDs, areas, row lengths, totals, route polyline + `length_m`
