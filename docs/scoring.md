...
| **25%** | Walking route | Target coverage + efficiency | Repo `route.geojson` vs hidden targets + passable geometry |
...
| Slice | Cap | Metric |
| --- | --- | --- |
| Coverage | **15%** | Organizers’ **hidden** list of inspection locations **and** waste. A target is visited if the route passes within **2 m**. |
| Efficiency | **10%** | `L_ref / L`, awarded only from **90% coverage** and then scaled by coverage. `L_ref` = shorter of the organizers’ route and the shortest **admitted** team route. |

**Instant 0 on this entire 25%** if either:

- more than **2%** of route length is outside passable inter-row areas ∪ authorised passages, or
- the route does not return to the official start within **5 m**.

Implications:

- Closed loop at `data/challenge/02_route/start.geojson` (5 m).
- Stay inside `interrow_area` polygons + `passages.geojson`. Never clip `forbidden.geojson` or walk through canopies.
- Visit **our** inspection points (gaps / missing planting) **and** waste. Hidden reference will be similar; coverage is scored against **theirs**.
- A short illegal shortcut zeros the whole criterion. Prefer a longer legal walk.
- Efficiency is only in play after ~90% coverage. Do not optimise length before coverage.

## 15% — Engineering quality (jury)

| Subscore | Points | They look at |
| --- | --- | --- |
| Architecture | 5 | Model → GIS stitch → CVAT → Marcaj → measurements → route → web |
...
3. **Protect the 25% route.** Legal graph, 2 m visit of gaps + waste, then shorten.
